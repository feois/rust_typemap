...
#[test]
fn test_dynamic_map() {
    let mut map: DynamicTypeMap = DynamicTypeMap::new();
    ...
    assert!(map.add::<C>("test"));
    assert_eq!(map.get::<A>(), Some(&0));
    assert_eq!(map.get::<B>(), Some(&2));
    assert_eq!(map.get::<C>(), Some(&"test"));
}

#[test]
fn test_dynamic_set() {
    let mut set = DynamicTypeSet::new();
...
fn test_dynamic_map_iter() {
    ...
    assert_eq!(map.get::<A>(), Some(&1));
    assert_eq!(map.get::<B>(), Some(&2));
    assert_eq!(map.get::<C>(), Some(&"test2"));
...
#[test]
fn test_dynamic_set_iter() {
    let mut map = DynamicTypeSet::new();
    ...
    assert!(a && b && c);
    assert!(matches!(map.get(), Some(&A(1))));
    assert!(matches!(map.get(), Some(&B("test2"))));
    assert!(matches!(map.get(), Some(&C)));
